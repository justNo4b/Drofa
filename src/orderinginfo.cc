...
void OrderingInfo::incrementCounterHistory(Color color, int pMove, int ppMove, PieceType pType, int to, int depth){
  int16_t current;
  int16_t bonus   = depth * depth;

  int pIndx  = (pMove & 0x7) + ((pMove >> 15) & 0x3f) * 6;
  int ppIndx = (ppMove & 0x7) + ((ppMove >> 15) & 0x3f) * 6;

  current = _counterMoveHistory[color][pIndx][pType][to];
  _counterMoveHistory[color][pIndx][pType][to] += 32 * bonus - current * abs(bonus) / 512;

  current = _followMoveHistory[color][ppIndx][pType][to];
  _followMoveHistory[color][ppIndx][pType][to] += 32 * bonus - current * abs(bonus) / 512;
}

void OrderingInfo::decrementCounterHistory(Color color, int pMoveIndx, int ppMoveIndx, PieceType pType, int to, int depth){
  int16_t current;
  int16_t bonus   = -1 * depth * depth;

  current = _counterMoveHistory[color][pMoveIndx][pType][to];
  _counterMoveHistory[color][pMoveIndx][pType][to] += 32 * bonus - current * abs(bonus) / 512;

  current = _followMoveHistory[color][ppMoveIndx][pType][to];
  _followMoveHistory[color][ppMoveIndx][pType][to] += 32 * bonus - current * abs(bonus) / 512;
...
int OrderingInfo::getCountermoveHistory(Color color, int pMoveIndx, int ppMoveIndx, PieceType pType, int to) const{
  return (_counterMoveHistory[color][pMoveIndx][pType][to]
        + _followMoveHistory[color][ppMoveIndx][pType][to])
        / 2;
}
