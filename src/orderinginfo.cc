...
void OrderingInfo::incrementHistory(bool incheck, Color color, PieceType pt, int to, int depth) {
  int16_t current = _history[color][incheck][pt][to];
  int16_t bonus   = depth * depth;
  _history[color][incheck][pt][to] += 32 * bonus - current * abs(bonus) / 512;
}

void OrderingInfo::decrementHistory(bool incheck, Color color, PieceType pt, int to, int depth) {
  int16_t current = _history[color][incheck][pt][to];
...
int OrderingInfo::getHistory(bool incheck, Color color, PieceType pt, int to) const {
  return _history[color][incheck][pt][to];
}
