...
void EvalTermInitiate(tValueHolder cTerms){
    ...
    c++;
    cTerms[c][OPENING] = opS(Eval::SPACE);
    cTerms[c][ENDGAME] = egS(Eval::SPACE);
    c++;
...
void InitCoefficients(featureCoeff coeff){
    ...
    coeff[i++] = ft.Space[WHITE] - ft.Space[BLACK];
...
void PrintTunedParams(tValueHolder currTerms, tValueHolder diffTerms){
    ...
    i++;
    EvalTermPrint("\nconst int  SPACE", currTerms[i][OPENING], currTerms[i][ENDGAME], diffTerms[i][OPENING], diffTerms[i][ENDGAME]);
    i++;
