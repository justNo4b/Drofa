...
void EvalTermInitiate(tValueHolder cTerms){
    ...
    c++;
    cTerms[c][OPENING] = opS(Eval::BISHOP_PROTECTOR);
    cTerms[c][ENDGAME] = egS(Eval::BISHOP_PROTECTOR);
    c++;
...
void InitCoefficients(featureCoeff coeff){
    ...
    coeff[i++] = ft.BishopProtector[WHITE] - ft.BishopProtector[BLACK];
...
void PrintTunedParams(tValueHolder currTerms, tValueHolder diffTerms){
    ...
    i++;
    EvalTermPrint("\nconst int  BISHOP_PROTECTOR", currTerms[i][OPENING], currTerms[i][ENDGAME], diffTerms[i][OPENING], diffTerms[i][ENDGAME]);
    i++;
