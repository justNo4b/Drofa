...
void HASH::pHASH_Store(U64 posKey, U64 whitePassers, U64 blackPassers, U64 whiteCandidates, U64 blackCandidates, int score){
  U64 index = posKey % pTableSize;
  if (index < pTableSize){
    pHASH[index] = pawn_HASH_Entry(posKey, whitePassers, blackPassers, whiteCandidates, blackCandidates, score);
  }
}
