...
void PSquareTable::init() {
    ...
    for (int i = 0; i < 64; i++){
      ...
      PIECE_VALUES[BLACK][ROOK][i] = Eval::ROOK_PSQT_BLACK[REFLECTED_SQUARE[i]];
      PIECE_VALUES[WHITE][ROOK][i] = Eval::ROOK_PSQT_BLACK[REFLECTED_SQUARE[_mir(i)]];

      PIECE_VALUES[BLACK][KNIGHT][i] = Eval::KNIGHT_PSQT_BLACK[REFLECTED_SQUARE[i]];
      PIECE_VALUES[WHITE][KNIGHT][i] = Eval::KNIGHT_PSQT_BLACK[REFLECTED_SQUARE[_mir(i)]];

      PIECE_VALUES[BLACK][BISHOP][i] = Eval::BISHOP_PSQT_BLACK[REFLECTED_SQUARE[i]];
      PIECE_VALUES[WHITE][BISHOP][i] = Eval::BISHOP_PSQT_BLACK[REFLECTED_SQUARE[_mir(i)]];

      PIECE_VALUES[BLACK][QUEEN][i] = Eval::QUEEN_PSQT_BLACK[REFLECTED_SQUARE[i]];
      PIECE_VALUES[WHITE][QUEEN][i] = Eval::QUEEN_PSQT_BLACK[REFLECTED_SQUARE[_mir(i)]];
