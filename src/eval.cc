  ...
  if (TRACK) ft.HangingPiece[KNIGHT][color] += (_popCount(pieces & eB->EnemyPawnAttackMap[color]));
    ...
    while (pieces) {
      ...
      if (eB->PossibleProtOutposts[color] & (ONE << square)){
        ...
        s += KNIGHT_PROT_OUTPOST_ADJ * _popCount(board.getPieces(otherColor, PAWN) & detail::TWO_AROUND[square]);
        eB->OutPostedLines[color] = eB->OutPostedLines[color] | detail::FILES[_col(square)];
        if (TRACK) ft.KnightOutProtBlack[color == WHITE ? REFLECTED_SQUARE[_mir(square)] : REFLECTED_SQUARE[square]][color]++;
        if (TRACK) ft.KnightProtOutAdj[color] += _popCount(board.getPieces(otherColor, PAWN) & detail::TWO_AROUND[square]);
      } else if (eB->PossibleGenOutposts[color] & (ONE << square)){
