...
inline int Eval::evaluateKING(const Board & board, Color color, evalBits * eB){
  ...
  bool isEnemyMajor = board.getPieces(otherColor, ROOK) != 0 || board.getPieces(otherColor, QUEEN) != 0;
  if (((file & ourPawns) == 0) && ((file & enemyPawns) == 0)){
    s += KING_OPEN_FILE[isEnemyMajor];
    if (TRACK) ft.KingOpenFile[isEnemyMajor][color]++;
  } else if ((file & ourPawns) == 0){
    s += KING_OWN_SEMI_FILE[isEnemyMajor];
    if (TRACK) ft.KingSemiOwnFile[isEnemyMajor][color]++;
  } else if ((file & enemyPawns) == 0){
    s += KING_ENEMY_SEMI_LINE[isEnemyMajor];
    if (TRACK) ft.KingSemiEnemyFile[isEnemyMajor][color]++;
  }
