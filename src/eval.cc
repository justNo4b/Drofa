...
inline int Eval::evaluateBISHOP(const Board & board, Color color, evalBits * eB){
  ...
  if (TRACK) ft.HangingPiece[BISHOP][color] += (_popCount(pieces & eB->EnemyPawnAttackMap[color]));
    ...
    while (pieces) {
      ...
      s += BISHOP_OWN_PAWNS[_popCount(ownPawns & (((ONE << square) & WHITE_SQUARES) ? WHITE_SQUARES : BLACK_SQUARES)) / 2];
      if (TRACK) ft.BishopOwnPawns[_popCount(ownPawns & (((ONE << square) & WHITE_SQUARES) ? WHITE_SQUARES : BLACK_SQUARES)) / 2][color]++;
