...
inline int Eval::PiecePawnInteraction(const Board &board, Color color, evalBits * eB){
  ...
  U64 kingNear = Attacks::getNonSlidingAttacks(KING, eB->EnemyKingSquare[color], WHITE);
  int unContestedFar  = _popCount(eB->AttackedSquares[color] & eB->EnemyKingZone[color] & ~eB->AttackedSquares[otherColor] & ~kingNear);
  int unContestedNear = _popCount(eB->AttackedSquares[color] & eB->EnemyKingZone[color] & ~eB->AttackedSquares[otherColor] &  kingNear);
  eB->KingAttackPower[color] += UNCONTESTED_FAR_ATTACK[std::min(unContestedFar, 3)];
  eB->KingAttackPower[color] += UNCONTESTED_KING_ATTACK[std::min(unContestedNear, 5)];
