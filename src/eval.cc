...
inline int Eval::evaluateBISHOP(const Board & board, Color color, evalBits * eB){
  ...
  U64 enPawns = board.getPieces(getOppositeColor(color), PAWN);
  ...
  if (TRACK) ft.HangingPiece[BISHOP][color] += (_popCount(pieces & eB->EnemyPawnAttackMap[color]));
    ...
    while (pieces) {
      ...
      s += BISHOP_ENEMY_PAWNS[_popCount(enPawns & (((ONE << square) & WHITE_SQUARES) ? WHITE_SQUARES : BLACK_SQUARES))];
      if (TRACK) ft.BishopEnPawns[_popCount(enPawns & (((ONE << square) & WHITE_SQUARES) ? WHITE_SQUARES : BLACK_SQUARES))][color]++;
