...
inline void Search::_updateBeta(bool isQuiet, const Move move, Color color, int pMove, int ppMove, int ply, int depth){
    ...
    _orderingInfo.incrementCounterHistory(color, pMove, ppMove, move.getPieceType(), move.getTo(), 2 * depth);
  }else{
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          if (isPmQuietCounter) _orderingInfo.incrementCounterHistory(board.getActivePlayer(), pMove, ppMove, move.getPieceType(), move.getTo(), depth);
        ...
        if (score > alpha) {
          ...
          if (isQuiet){
            ...
            _orderingInfo.decrementCounterHistory(board.getActivePlayer(), pMoveIndx, ppMoveIndx, move.getPieceType(), move.getTo(), dBonus);
          }else{
