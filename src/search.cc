...
int Search::_negaMax(const Board &board, pV *up_pV, int depth, int alpha, int beta, bool sing, bool cutNode) {
  ...
  int  qCount = 0;
  int historyPunishment = std::max(0, depth - (statEVAL < alpha) - (!TTmove && depth >= 4));
  int historyAward = depth + 2 * (statEVAL < alpha);
  while (movePicker.hasNext()) {
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, move, board.getActivePlayer(), pMove, ply, historyAward);
        ...
        if (score > alpha) {
          ...
          if (isQuiet){
            _orderingInfo.decrementHistory(board.getActivePlayer(), move.getFrom(), move.getTo(), historyPunishment);
            _orderingInfo.decrementCounterHistory(board.getActivePlayer(), pMove, move.getPieceType(), move.getTo(), historyPunishment);
          }else{
            _orderingInfo.decrementCapHistory(move.getPieceType(), move.getCapturedPieceType(), move.getTo(), historyPunishment);
          }
        }
      }
