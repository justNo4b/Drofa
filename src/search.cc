    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          int hBonus = (depth + 2 * (statEVAL < alpha)) + (!TTmove && depth >= 10);
        ...
        if (score > alpha) {
          ...
          int hBonus = depth - (statEVAL < alpha);
          if (isQuiet){
            _orderingInfo.decrementHistory(board.getActivePlayer(), move.getFrom(), move.getTo(), hBonus);
            _orderingInfo.decrementCounterHistory(board.getActivePlayer(), pMove, move.getPieceType(), move.getTo(), hBonus);
          }else{
            _orderingInfo.decrementCapHistory(move.getPieceType(), move.getCapturedPieceType(), move.getTo(), hBonus);
          }
        }
      }
