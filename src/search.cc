...
inline void Search::_updateBeta(bool isQuiet, const Move move, Color color, int pType, int pTo, int ply, int depth){
	if (isQuiet) {
    _orderingInfo.updateKillers(ply, move);
    _orderingInfo.incrementHistory(color, move.getFrom(), move.getTo(), depth);
    _orderingInfo.updateCounterMove(color, pType, pTo, move.getMoveINT());
    _orderingInfo.incrementCounterHistory(color, pType + 6 * move.getPieceType(), pTo, move.getTo(), depth);
  }else{
  ...
  if (probedHASHentry.Flag != NONE){
    ...
    if (probedHASHentry.depth >= depth && !pvNode && !sing){
      ...
      if (probedHASHentry.Flag == BETA && hashScore >= beta){
        _updateBeta(quietTT, hashedMove, board.getActivePlayer(), pMovePiece, pMoveTo, ply, depth);
        return beta;
      }
    }
  }
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        int cmHistory     = isQuiet ?
                            _orderingInfo.getCountermoveHistory(board.getActivePlayer(), pMovePiece + 6 * move.getPieceType(), pMoveTo, move.getTo())
                            : 0;
        int tDepth = depth;
        // 6. EXTENTIONS
        //
        ...
        if (doLMR){
          ...
          reduction -= 2 * (move.getMoveINT() == _orderingInfo.getCounterMoveINT(board.getActivePlayer(), pMovePiece, pMoveTo) ||
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, move, board.getActivePlayer(), pMovePiece, pMoveTo, ply, (depth + 2 * (statEVAL < alpha)));
        ...
        if (score > alpha) {
          ...
          if (isQuiet){
            ...
            _orderingInfo.decrementCounterHistory(board.getActivePlayer(), pMovePiece + 6 * move.getPieceType(), pMoveTo, move.getTo(), dBonus);
          }else{
