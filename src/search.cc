    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (doLMR){
          ...
          reduction = _lmr_R_array[std::min(33, tDepth)][std::min(33, LegalMoveCount)];
        ...
        if (doLMR){
          if (score > alpha){
            ...
            if (reduction > 0) _orderingInfo.incrementCounterHistory(board.getActivePlayer(), pMove, move.getPieceType(), move.getTo(), depth / 2);
          }
