...
inline void Search::_updateBeta(bool isQuiet, bool isPv, const Move move, Color color, int pMove, int ply, int depth){
	if (isQuiet) {
    _orderingInfo.updateKillers(ply, move);
    _orderingInfo.incrementHistory(color, move.getFrom(), move.getTo(), depth, isPv);
...
int Search::_rootMax(const Board &board, int alpha, int beta, int depth) {
  ...
  MovePicker movePicker(&_orderingInfo, &board, legalMoves, hashMove, board.getActivePlayer(), 0, 0, true);
...
int Search::_negaMax(const Board &board, pV *up_pV, int depth, int alpha, int beta, bool sing, bool cutNode) {
  ...
  if (probedHASHentry.Flag != NONE){
    ...
    if (probedHASHentry.depth >= depth && !pvNode && !sing){
      ...
      if (probedHASHentry.Flag == BETA && hashScore >= beta){
        _updateBeta(quietTT, pvNode, hashedMove, board.getActivePlayer(), pMove, ply, depth);
        return beta;
      }
    }
  }
  ...
  MoveGen movegen(board, false);
  MoveList * legalMoves = movegen.getMoves();
  MovePicker movePicker(&_orderingInfo, &board, legalMoves, hashedMove.getMoveINT(), board.getActivePlayer(), ply, pMove, pvNode);
  ...
  while (movePicker.hasNext()) {
    ...
    int  moveHistory  = isQuiet ?
                        _orderingInfo.getHistory(board.getActivePlayer(), move.getFrom(), move.getTo(), pvNode) :
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, pvNode, move, board.getActivePlayer(), pMove, ply, (depth + 2 * (statEVAL < alpha)));
        ...
        if (score > alpha) {
          ...
          int dBonus = std::max(0, depth - (statEVAL < alpha) - (!TTmove && depth >= 4) + (pMoveScore < -8192));
          if (isQuiet){
            _orderingInfo.decrementHistory(board.getActivePlayer(), move.getFrom(), move.getTo(), dBonus, pvNode);
...
int Search::_qSearch(const Board &board, int alpha, int beta) {
  ...
  MoveList * legalMoves = movegen.getMoves();
  MovePicker movePicker(&_orderingInfo, &board, legalMoves, 0, board.getActivePlayer(), MAX_PLY, 0, pvNode);
