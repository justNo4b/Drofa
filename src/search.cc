  ...
  while (movePicker.hasNext()) {
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        int fmHistory     = isQuiet ? _orderingInfo.getFollowmoveHistory(ppMove, move.getPieceType(), move.getTo()) : 0;
        int tDepth = depth;
        // 6. EXTENTIONS
        //
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, move, board.getActivePlayer(), pMove, ppMove, ply, (depth + (statEVAL < alpha)));
...
int Search::_qSearch(const Board &board, int alpha, int beta) {
  ...
  MovePicker movePicker
      (&_orderingInfo, &board, &legalMoves, 0, board.getActivePlayer(), MAX_PLY, 0, 0);
