    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, move, board.getActivePlayer(), pMove, ply, (tDepth + 2 * (statEVAL < alpha)));
          // Award counter-move history additionally if we refuted special quite previous move
          if (pMoveScore >= 50000 && pMoveScore <= 200000)
            _orderingInfo.incrementCounterHistory(board.getActivePlayer(), pMove, move.getPieceType(), move.getTo(), tDepth);
