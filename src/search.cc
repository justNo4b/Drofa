    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, move, board.getActivePlayer(), pMove, ply, (depth + 2 * (statEVAL < alpha && !AreWeInCheck)));
        ...
        if (score > alpha) {
          ...
          int dBonus = std::max(0, depth - (statEVAL < alpha && !AreWeInCheck) - (!TTmove && depth >= 4));
          if (isQuiet){
