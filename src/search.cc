  ...
  if (probedHASHentry.Flag != NONE){
    ...
    if (probedHASHentry.depth >= depth && !pvNode && !sing){
      ...
      if (probedHASHentry.Flag == BETA && hashScore >= beta){
        _updateBeta(quietTT, hashedMove, board.getActivePlayer(), pMove, ply, hupDepth);
        return beta;
      }
    }
  }
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score >= beta) {
          ...
          _updateBeta(isQuiet, move, board.getActivePlayer(), pMove, ply, (hupDepth + (statEVAL < alpha)));
        ...
        if (score > alpha) {
          ...
          if (isQuiet){
            _orderingInfo.decrementHistory(board.getActivePlayer(), move.getFrom(), move.getTo(), hupDepth);
            _orderingInfo.decrementCounterHistory(pMove, move.getPieceType(), move.getTo(), hupDepth);
          }else{
            _orderingInfo.decrementCapHistory(move.getPieceType(), move.getCapturedPieceType(), move.getTo(), hupDepth);
          }
        }
      }
