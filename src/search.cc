      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score > alpha) {
          ...
          if (isQuiet){
            ...
            _orderingInfo.decrementCounterHistory(board.getActivePlayer(), pMove, move.getPieceType(), move.getTo(), depth);
            _orderingInfo.decrementFollowHistory(ppMove, move.getPieceType(), move.getTo(), depth);
          }else{
