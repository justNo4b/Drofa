...
int Search::_negaMax(const Board &board, pV *up_pV, int depth, int alpha, int beta, bool singSearch, bool cutNode) {
  ...
  if (depth >= 5 && (!ttNode || ttMove == 0) && pMove != 0 && !singSearch)
    depth--;
    ...
    bool doLMR = false;
      ...
      if (!movedBoard.colorIsInCheck(movedBoard.getInactivePlayer())){
        ...
        if (score > alpha) {
          ...
          int dBonus = std::max(0, depth - (nodeEval < alpha) - ((!ttNode || ttMove == 0) && depth >= 4) + (pMoveScore < -HALFMAX_HISTORY_SCORE) + cutNode);
          if (isQuiet){
